...
export class CartService {
  ...
  addToCart(food: Menu): void {
    ...
    let cartItem = this.cart.items.find(item => item.food.menuId === food.menuId);
    if(cartItem)
    {
  ...
  removeFromCart(foodId: number): void {
    this.cart.items = this.cart.items.filter(item => item.food.menuId !== foodId);  
    this.setCartToLocalStorage();  
  }
  ...
  changeQuantity(foodId: number, quantity: number) {
    ...
    let cartItem = this.cart.items.find(item => item.food.menuId === foodId);
